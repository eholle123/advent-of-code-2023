...
def predict_previous_value_in_sequence(
    sequence_tree: List[List[int]], first_value: int
) -> List[List[int]]:
    previous_value = 0
...
def get_predicted_previous_values(
    oasis_and_sand_sensor_data: List[List[int]],
) -> List[int]:
    predicted_values = []
...
if __name__ == "__main__":
    ...
    predicted_previous_values = get_predicted_previous_values(
        oasis_and_sand_sensor_data
    )
